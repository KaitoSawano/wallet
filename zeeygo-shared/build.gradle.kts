...
/**
 * Generates a `BuildVersions` file in zeeygo-shared/build/generated-src containing the current git commit and the lightning-kmp version.
...
val buildVersionsTask by tasks.registering(Sync::class) {
    from(
        resources.text.fromString(
            """
            |package fr.acinq.zeeygo.shared
            |
            |object BuildVersions {
            |    const val ZEEYGO_COMMIT = "${gitCommitHash()}"
        ...
        it.binaries {
            ...
            configureEach {
                it.compilations.all {
                    ...
                    // workaround for IndexOutOfBoundsException with zeeygo-shared:linkReleaseFrameworkIosArm64 in iOS Archive build
...
sqldelight {
    databases {
        create("ChannelsDatabase") {
            packageName.set("fr.acinq.zeeygo.db.sqldelight")
...
if (includeAndroid) {
    extensions.configure<com.android.build.gradle.LibraryExtension>("android") {
        namespace = "fr.acinq.zeeygo.shared"
        compileSdk = 34
        defaultConfig {
            minSdk = 26
