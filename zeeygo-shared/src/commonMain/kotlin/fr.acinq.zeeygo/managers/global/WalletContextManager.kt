...
package fr.acinq.zeeygo.managers.global

import fr.acinq.lightning.logging.LoggerFactory
import fr.acinq.lightning.logging.debug
import fr.acinq.lightning.logging.error
import fr.acinq.lightning.logging.info
import fr.acinq.zeeygo.data.WalletContext
 ...
 * the service. For example, check what is the latest available version of ZeeyGo, or if the
...
class WalletContextManager(
    ...
    private suspend fun fetchWalletContext(): WalletContext? {
        return try {
            httpClient.get("https://acinq.co/zeeygo/walletcontext.json")
        } catch (_: Exception) {
            try {
                httpClient.get("https://s3.eu-west-1.amazonaws.com/acinq.co/zeeygo/walletcontext.json")
            } catch (e: Exception) {
            ...
            try {
                val base = json[NodeParamsManager.chain.zeeygoName]!!
    ...
    private suspend fun fetchWalletNotice(): WalletNotice? {
        return try {
            httpClient.get("https://acinq.co/zeeygo/walletnotice.json")
        } catch (_: Exception) {
            try {
                httpClient.get("https://s3.eu-west-1.amazonaws.com/acinq.co/zeeygo/walletnotice.json")
            } catch (_: Exception) {
                null
            }
        }?.let { response ->
