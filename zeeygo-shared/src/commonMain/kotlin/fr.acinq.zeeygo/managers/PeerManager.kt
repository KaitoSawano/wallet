...
class PeerManager(
    ...
    constructor(business: ZeeyGoBusiness) : this(
        ...
        feerateManager = business.zeeygoGlobal.feerateManager,
    ...
    /** Forward compatibility check. [UpgradeRequired] is sent by the peer when an old version of ZeeyGo restores a wallet that has been used with new channel types. */
    ...
    val finalWallet = peerState.filterNotNull().flatMapLatest { peer ->
        combine(peer.currentTipFlow.filterNotNull(), peer.zeeygoFinalWallet.wallet.walletStateFlow) { currentBlockHeight, wallet ->
    ...
    val swapInWallet = peerState.filterNotNull().flatMapLatest { peer ->
        combine(peer.currentTipFlow.filterNotNull(), peer.channelsFlow, peer.zeeygoSwapInWallet.wallet.walletStateFlow) { currentBlockHeight, channels, swapInWallet ->
...
/** The peer's swap-in wallet for ZeeyGo is always not null, because the client is always an [IElectrumClient] (see how this Peer is built in `PeerManager.init`). */
val Peer.zeeygoSwapInWallet: SwapInWallet
    get() = this.swapInWallet!!

/** The peer's final wallet for ZeeyGo is always not null, because the client is always an [IElectrumClient] (see how this Peer is built in `PeerManager.init`). */
val Peer.zeeygoFinalWallet: FinalWallet
