...
import fr.acinq.zeeygo.managers.global.NetworkState
...
class AppConnectionsDaemon(
    ...
    private val zeeygoGlobal: ZeeyGoGlobal,
    ...
    constructor(business: ZeeyGoBusiness) : this(
        ...
        zeeygoGlobal = business.zeeygoGlobal,
    ...
    init {
        ...
        // Internet monitor
        launch {
            zeeygoGlobal.networkMonitor.start()
            zeeygoGlobal.networkMonitor.networkState.collect {
                ...
                when {
                    it.internetIsAvailable && it.disconnectCount <= 0 -> {
                        if (!httpControlFlowEnabled) {
                            ...
                            zeeygoGlobal.enableNetworkAccess()
                        }
                    }
                    else -> {
                        if (httpControlFlowEnabled) {
                            httpControlFlowEnabled = false
                            zeeygoGlobal.disableNetworkAccess()
                        }
                    }
                }
            }
