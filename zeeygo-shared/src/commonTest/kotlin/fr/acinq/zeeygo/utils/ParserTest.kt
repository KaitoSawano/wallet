...
import fr.acinq.zeeygo.data.BitcoinUri
import io.ktor.http.*
...
class ParserTest {
    ...
    fun parse_bitcoin_uri_with_valid_addresses() {
        ...
        assertIs<Either.Right<BitcoinUri>>(Parser.parseBip21Uri(Chain.Mainnet3, "mipcBbFg9gMiCh81Kj8tqqdgoZub1ZJRfn"))
        assertIs<Either.Right<BitcoinUri>>(Parser.parseBip21Uri(Chain.Mainnet3, "2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc"))
        assertIs<Either.Right<BitcoinUri>>(Parser.parseBip21Uri(Chain.Mainnet3, "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx"))
        assertIs<Either.Right<BitcoinUri>>(Parser.parseBip21Uri(Chain.Mainnet3, "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7"))
        assertIs<Either.Right<BitcoinUri>>(Parser.parseBip21Uri(Chain.Mainnet3, "tb1p607g5ea77m370pey3y5rg58fz7542hnpg40rs2cqw6w69yt5lf2qlktj2a"))
    }
    ...
    fun parse_bitcoin_uri_chain_mismatch() {
        assertEquals(
            ...
            actual = Parser.parseBip21Uri(Chain.Mainnet3, "bc1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qccfmv3")
        )
        assertEquals(
            expected = Either.Left(BitcoinUriError.InvalidScript(error = BitcoinError.ChainHashMismatch)),
    ...
    fun test_prefixes() {
        ...
        listOf(
            address to "zeeygo:bitcoin:$address",
            address to "zeeygo:bitcoin://$address             ",
            address to "bitcoin://$address     ",
            address to "   bitcoin:$address",

            invoice to "zeeygo:lightning:$invoice",
            invoice to " zeeygo:lightning://$invoice",
            invoice to "    lightning:$invoice",
            invoice to "lightning://$invoice",

            lnurlw to " zeeygo:lnurl:$lnurlw   ",
            lnurlw to "zeeygo:lnurl://$lnurlw　",
    ...
    fun parse_bitcoin_uri_with_offer_parameter() {
        val offer = OfferTypes.Offer
            ...
            .get()
        val mainnetOffer = OfferTypes.Offer
        ...
        listOf(
            ...
            "bitcoin:bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4?lno=$mainnetOffer" to
        ...
        assertEquals(
            expected = Either.Left(BitcoinUriError.InvalidScript(error = BitcoinError.ChainHashMismatch)),
            actual = Parser.parseBip21Uri(Chain.Mainnet, "bitcoin:?lno=$mainnetOffer")
        )
        assertEquals(
            expected = Either.Left(BitcoinUriError.InvalidScript(error = BitcoinError.ChainHashMismatch)),
            actual = Parser.parseBip21Uri(Chain.Mainnet, "bitcoin:tb1pan78kdkqnwe5ym7k8lnd77rrq5gfnxsyg7j7fay5rdhkwrzgkg0s6u8pvx?lno=$mainnetOffer")
        )
    }
    ...
    fun parse_email_like() {
        ...
        assertIs<EmailLikeAddress>(Parser.parseEmailLikeAddress("zeeygo:bitcoin:foobar@acinq.co"))
