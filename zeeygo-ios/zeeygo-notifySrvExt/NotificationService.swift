...
class NotificationService: UNNotificationServiceExtension {
	...
	private var zeeygoStarted: Bool = false
	private var srvExtDone: Bool = false
	
	private var business: ZeeyGoBusiness? = nil
	...
	private func startConnectionTimer() {
		...
		guard let groupPrefs = ZeeyGoManager.shared.groupPrefs() else {
	...
	private func didReceiveXpcMessage(_ msg: XpcMessage) {
		...
		if msg == .available {
			...
			// The main zeeygo app is running.
			log.info("didReceiveXpcMessage: available (main zeeygo app is running)")
	...
	private func startZeeyGo() {
		log.trace(#function)
		assertMainThread()
		
		guard !srvExtDone else {
			log.info("startZeeyGo(): ignoring: srvExtDone")
			return
		}
		guard !zeeygoStarted else {
			log.warning("startZeeyGo(): ignoring: already started")
			return
		}
		zeeygoStarted = true
		
		let newBusiness = ZeeyGoManager.shared.setupBusiness(targetNodeIdHash)
	...
	private func stopZeeyGo() {
		log.trace(#function)
		assertMainThread()
		
		guard zeeygoStarted else {
			log.info("stopZeeyGo(): ignoring: already stopped")
			return
		}
		zeeygoStarted = false
		
		ZeeyGoManager.shared.teardownBusiness()
		business = nil
	}
	...
	private func finish() {
		...
		// and to do that we call ZeeyGoManager.groupPrefs().
	...
	private func updateNotificationContent_receivedPayment(
		...
		let groupPrefs = ZeeyGoManager.shared.groupPrefs()
	...
	private func updateNotificationContent_pendingSettlement(
		...
		content.title = String(localized: "Please start ZeeyGo", comment: "")
	...
	private func formatAmount(_ groupPrefs: GroupPrefs_Wallet, msat: Int64) -> String {
		...
		let exchangeRate = ZeeyGoManager.shared.exchangeRate(fiatCurrency: fiatCurrency)
