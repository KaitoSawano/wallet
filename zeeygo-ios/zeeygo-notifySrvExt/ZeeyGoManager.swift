...
fileprivate let filename = "ZeeyGoManager"
#if DEBUG && true
 ...
 * # Architecture notes:
 *
 * In a previous implementation, we created the `ZeeyGoBusiness` instance once,
...
class ZeeyGoManager {
	...
	public static let shared = ZeeyGoManager()
	
	private static let zeeygoGlobal = ZeeyGoGlobal(ctx: PlatformContext.default)
	
	private var business: ZeeyGoBusiness? = nil
	private var oldBusiness: ZeeyGoBusiness? = nil
	...
	public func setupBusiness(_ target: String?) -> ZeeyGoBusiness {
		log.trace(#function)
		assertMainThread()
		...
		let newBusiness = ZeeyGoBusiness(zeeygoGlobal: ZeeyGoManager.zeeygoGlobal)

		newBusiness.zeeygoGlobal.networkMonitor.disable()
		newBusiness.zeeygoGlobal.currencyManager.disableAutoRefresh()
	...
	private func unlock(_ recoveryPhrase: RecoveryPhrase?, _ targetNodeIdHash: String?) {
		...
		business.zeeygoGlobal.currencyManager.refreshAll(targets: [primaryFiatCurrency], force: false)
		Task { @MainActor [business, weak self] in
			for await rates in business.zeeygoGlobal.currencyManager.ratesSequence() {
