...
In a previous version of ZeeyGo we used "silent" push notifications to wake up the zeeygo app, and allow it to receive payments when in the background. However, there were multiple problems with this.
...
Second, we use mach ports to send ping/pong messages between the container app & app extension. This allows the app extension to yield to the main zeeygo app, and allows it to handle the payment.
