	...
	func section_enabled() -> some View {
		...
		Section {
			VStack(alignment: HorizontalAlignment.leading, spacing: 0) {
				...
				Label {
					Text("(even if ZeeyGo isn't running)").foregroundColor(.secondary)
				} icon: {
					invisibleImage()
				}
				.font(.callout)
	...
	func section_disabled() -> some View {
		...
		Section {
			VStack(alignment: HorizontalAlignment.leading, spacing: 0) {
				...
				Label {
					Text("You can only receive payments if ZeeyGo is running in the foreground.")
						.font(.callout)
				} icon: {
				...
				Label {
					VStack(alignment: HorizontalAlignment.leading, spacing: 8) {
						...
						Text("(even if ZeeyGo isn't running)")
					...
					VStack(alignment: HorizontalAlignment.leading, spacing: 10) {
						...
						HStack(alignment: VerticalAlignment.top, spacing: 8) {
							...
							Button {
								...
								Text("Open iOS Settings for ZeeyGo")
							}
						}
						HStack(alignment: VerticalAlignment.top, spacing: 8) {
							Text(verbatim: "*").bold()
