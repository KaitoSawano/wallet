...
struct BizNotificationCell: View {
	...
	let item: ZeeyGoShared.NotificationsManager.NotificationItem
	...
	@ViewBuilder
	var body: some View {
		if let reason = item.notification as? ZeeyGoShared.Notification.PaymentRejected {

			if let reason = reason as? ZeeyGoShared.Notification.PaymentRejected.OverAbsoluteFee {
				body_paymentRejected_overFee(Either.Left(reason))
				
			} else if let reason = reason as? ZeeyGoShared.Notification.PaymentRejected.OverRelativeFee {
		...
		} else if let reason = item.notification as? ZeeyGoShared.WatchTowerOutcome {

			if let reason = reason as? ZeeyGoShared.WatchTowerOutcome.Nominal {
				body_watchTowerSuccess(reason)

			} else if let reason = reason as? ZeeyGoShared.WatchTowerOutcome.RevokedFound {
				body_watchTowerRevoked(reason)

			} else if let reason = reason as? ZeeyGoShared.WatchTowerOutcome.Unknown {
	...
	func body_paymentRejected_overFee(
		_ either: Either<
			ZeeyGoShared.Notification.PaymentRejected.OverAbsoluteFee,
			ZeeyGoShared.Notification.PaymentRejected.OverRelativeFee
		>
	) -> some View {
	...
	func body_paymentRejected(
		_ reason: ZeeyGoShared.Notification.PaymentRejected
	) -> some View {
	...
	func body_watchTowerSuccess(
		_ reason: ZeeyGoShared.WatchTowerOutcome.Nominal
	) -> some View {
	...
	func body_watchTowerRevoked(
		_ reason: ZeeyGoShared.WatchTowerOutcome.RevokedFound
	) -> some View {
	...
	func body_watchTowerUnknown(
		_ reason: ZeeyGoShared.WatchTowerOutcome.Unknown
	) -> some View {
