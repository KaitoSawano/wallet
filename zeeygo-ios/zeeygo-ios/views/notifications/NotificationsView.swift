...
struct NotificationsView : View {
	...
	@State var bizNotifications_payment: [ZeeyGoShared.NotificationsManager.NotificationItem] = []
	@State var bizNotifications_watchtower: [ZeeyGoShared.NotificationsManager.NotificationItem] = []
	...
	func bizNotificationsChanged(_ list: [ZeeyGoShared.NotificationsManager.NotificationItem]) {
		log.trace("bizNotificationsChanges()")
		
		bizNotifications_payment = list.filter({ item in
			if let _ = item.notification as? ZeeyGoShared.Notification.PaymentRejected {
				return true
			} else {
				return false
			}
		...
		bizNotifications_watchtower = list.filter({ item in
			if let watchTower = item.notification as? ZeeyGoShared.WatchTowerOutcome {
				// Remove "Nominal" notifications (which just mean everything is working as expected)
				return !(watchTower is ZeeyGoShared.WatchTowerOutcome.Nominal)
			} else {
				return false
			}
		})
	...
	func openMempoolFullURL() {
		...
		if let url = URL(string: "https://zeeygo.acinq.co/faq#high-mempool-size-impacts") {
			openURL(url)
		}
	}
	...
	func deleteNotification(_ item: ZeeyGoShared.NotificationsManager.NotificationItem) {
		log.trace(#function)
