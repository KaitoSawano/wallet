...
fileprivate let filename = "KotlinPublishers+ZeeyGo"
#if DEBUG && true
...
// MARK: -
extension ZeeyGoShared.NotificationsManager {

	struct NotificationItem: Identifiable {
		let ids: Set<Lightning_kmpUUID>
		let notification: ZeeyGoShared.Notification
			...
			.map { originalArray in
				let transformedArray: [NotificationItem] = originalArray.compactMap { value in
					guard
						...
						let notification = pair.second as? ZeeyGoShared.Notification
					else {
						return nil
					}
