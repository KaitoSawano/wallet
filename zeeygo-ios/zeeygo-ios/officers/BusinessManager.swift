...
/// Short-hand for `BusinessManager.zeeygoGlobal`
///
var BizGlobal: ZeeyGoGlobal {
	return BusinessManager.zeeygoGlobal
}
...
/// Manages the `ZeeyGoBusiness` instance, which is the shared logic written in Kotlin Multiplatform.
///
class BusinessManager {

	public static let zeeygoGlobal = ZeeyGoGlobal(ctx: PlatformContext.default)
	
	/// There are some places in the code where we need to access the mainnet state from a background thread.
	/// This is problematic because calling into Kotlin via `business.chain.isMainnet()`
	...
	public let showMainnetBackground: Bool
	
	/// The associated business instance.
	///
	public let business: ZeeyGoBusiness
	...
	init() {
		...
		business = ZeeyGoBusiness(zeeygoGlobal: BusinessManager.zeeygoGlobal)
		isMainnet = !business.chain.isMainnet()
		showMainnetBackground = !business.chain.isMainnet()
	...
	private func setup(_ walletId: WalletIdentifier) {
		...
		Task { @MainActor [self] in
			...
			for await newInfo in peer.zeeygoSwapInWallet.swapInAddressSequence() {
				if let newInfo {
