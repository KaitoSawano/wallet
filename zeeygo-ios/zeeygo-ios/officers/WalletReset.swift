...
/// In particular, the vast majority of the UI is deeply connected with the ZeeyGoShared instance.
/// And we're trying to reset ZeeyGoShared instance, along with the UI.
...
///    ensuring that no references to the previous ZeeyGoShared instance are being used.
///
class WalletReset {
	...
	*/
	...
	/**
	 * The next step is to shutdown the zeeygo-shared layer (i.e. the Kotlin Multiplatform stuff).
	...
	private func step3() {
		...
		guard
			let groupDir = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.co.acinq.zeeygo")
		else {
			return step4()
		}
		...
		let chainName: String = Biz.business.chain.zeeygoName
