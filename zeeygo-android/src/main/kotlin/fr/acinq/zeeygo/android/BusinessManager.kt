...
package fr.acinq.zeeygo.android
...
import fr.acinq.zeeygo.android.components.wallet.WalletAvatars
import fr.acinq.zeeygo.android.services.InflightPaymentsWatcher
import fr.acinq.zeeygo.android.utils.SystemNotificationHelper
import fr.acinq.zeeygo.android.utils.datastore.DataStoreManager
import fr.acinq.zeeygo.android.utils.datastore.InternalPrefs
import fr.acinq.zeeygo.android.utils.datastore.UserPrefs
import fr.acinq.zeeygo.android.utils.datastore.UserWalletMetadata
import fr.acinq.zeeygo.android.utils.datastore.getByWalletIdOrDefault
import fr.acinq.zeeygo.data.StartupParams
import fr.acinq.zeeygo.data.inFlightPaymentsCount
import fr.acinq.zeeygo.managers.AppConnectionsDaemon
import fr.acinq.zeeygo.managers.NodeParamsManager
import fr.acinq.zeeygo.managers.PeerManager
import fr.acinq.zeeygo.managers.WalletManager
import fr.acinq.zeeygo.managers.global.CurrencyManager
import fr.acinq.zeeygo.utils.MnemonicLanguage
...
sealed class StartBusinessResult {
    data class Success(val walletInfo: WalletManager.WalletInfo, val business: ZeeyGoBusiness): StartBusinessResult()
...
object BusinessManager {
    ...
    private val application by lazy { appContext as ZeeyGoApplication }
    ...
    suspend fun startNewBusiness(words: List<String>, isHeadless: Boolean): StartBusinessResult = startupMutex.withLock {
        ...
        val business = ZeeyGoBusiness(application.zeeygoGlobal)
        ...
        return try {
            ...
            if (lastVersionUsed == null) {
                // lastUsedAppCode was added in version 99, and is set up during the wallet creation. So if it's null, this ZeeyGo was installed prior v99 and we can show a patch note
            ...
            business.zeeygoGlobal.currencyManager.startMonitoringCurrencies(walletId = walletId.nodeIdHash, currencies = preferredCurrencies)
            ...
            eventsMonitoringJobs[walletId] = BusinessMonitorJobs(
                monitorHeadlessPaymentsJob = if (isHeadless) {
                    scope.launch { monitorPaymentsWhenHeadless(walletId, walletMetadata, business.nodeParamsManager, application.zeeygoGlobal.currencyManager, userPrefs) }
                } else null,
    ...
    private fun doStopBusiness(walletId: WalletId, running: BusinessRunning) {
        ...
        application.zeeygoGlobal.currencyManager.stopMonitoringForWallet(walletId.nodeIdHash)
    }
    ...
    private suspend fun monitorFcmToken(business: ZeeyGoBusiness) {
