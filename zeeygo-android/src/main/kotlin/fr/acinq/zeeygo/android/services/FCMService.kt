...
class FCMService : FirebaseMessagingService() {
        ...
        when {
            ...
            remoteMessage.priority != RemoteMessage.PRIORITY_HIGH -> {
                ...
                serviceScope.launch {
                    ...
                    val walletMetadata = (application as ZeeyGoApplication).globalPrefs.getAvailableWalletsMeta.first().getByWalletIdOrDefault(walletId)
                    when (reason) {
                ...
                log.debug("starting zeeygo foreground service with reason=$reason")
                startZeeyGoForegroundService(reason, nodeIdHash)
            }
        }
    }

    private fun startZeeyGoForegroundService(reason: String?, nodeIdHash: String) {
    ...
    override fun onNewToken(token: String) {
        serviceScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val globalPrefs = (applicationContext as ZeeyGoApplication).globalPrefs
