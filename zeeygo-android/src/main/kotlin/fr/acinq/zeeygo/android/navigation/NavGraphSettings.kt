...
package fr.acinq.zeeygo.android.navigation
...
import fr.acinq.zeeygo.android.AppViewModel
import fr.acinq.zeeygo.android.NoticesViewModel
import fr.acinq.zeeygo.android.PaymentsViewModel
import fr.acinq.zeeygo.android.WalletId
import fr.acinq.zeeygo.android.payments.history.PaymentsExportView
import fr.acinq.zeeygo.android.payments.history.PaymentsHistoryView
import fr.acinq.zeeygo.android.payments.send.liquidity.RequestLiquidityView
import fr.acinq.zeeygo.android.settings.AboutView
import fr.acinq.zeeygo.android.settings.AppAccessSettings
import fr.acinq.zeeygo.android.settings.troubleshoot.TroubleshootingView
import fr.acinq.zeeygo.android.settings.DisplayPrefsView
import fr.acinq.zeeygo.android.settings.ExperimentalView
import fr.acinq.zeeygo.android.settings.ForceCloseView
import fr.acinq.zeeygo.android.settings.NotificationsView
import fr.acinq.zeeygo.android.settings.PaymentSettingsView
import fr.acinq.zeeygo.android.settings.reset.ResetWallet
import fr.acinq.zeeygo.android.settings.SettingsContactsView
import fr.acinq.zeeygo.android.settings.SettingsView
import fr.acinq.zeeygo.android.settings.TorConfigView
import fr.acinq.zeeygo.android.settings.displayseed.DisplaySeedView
import fr.acinq.zeeygo.android.settings.electrum.ElectrumView
import fr.acinq.zeeygo.android.settings.fees.AdvancedIncomingFeePolicy
import fr.acinq.zeeygo.android.settings.fees.LiquidityPolicyView
...
fun NavGraphBuilder.settingsNavGraph(navController: NavController, appViewModel: AppViewModel) {
    ...
    businessComposable(Screen.BusinessNavGraph.Notifications.route, appViewModel, deepLinks = listOf(navDeepLink { uriPattern = "zeeygo:notifications/{walletId}" })) { backStackEntry, walletId, business ->
    ...
    businessComposable(Screen.BusinessNavGraph.LiquidityPolicy.route, appViewModel, deepLinks = listOf(navDeepLink { uriPattern = "zeeygo:liquiditypolicy" })) { _, _, business ->
        LiquidityPolicyView(
            business = business,
            onBackClick = { navController.popBackStackOrHome() },
    ...
    businessComposable(Screen.BusinessNavGraph.LiquidityRequest.route, appViewModel, deepLinks = listOf(navDeepLink { uriPattern = "zeeygo:requestliquidity" })) { _, walletId, business ->
