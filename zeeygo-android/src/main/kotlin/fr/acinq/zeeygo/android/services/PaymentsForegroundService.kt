...
import fr.acinq.zeeygo.android.StartBusinessResult
import fr.acinq.zeeygo.android.WalletId
import fr.acinq.zeeygo.android.security.DecryptSeedResult
import fr.acinq.zeeygo.android.security.SeedManager
...
/**
 * This foreground service starts a [ZeeyGoBusiness] upon receiving FCM payment messages from the LSP. Being a foreground
 * service, it needs to display a foreground notification visible by the user.
 *
 * This service allows ZeeyGo to receive payments (or settle pending payments) even when it's closed or in the background.
 */
            ...
            walletId == null -> {
                ...
                when (val result = SeedManager.loadAndDecrypt(applicationContext)) {
                    ...
                    is DecryptSeedResult.Failure -> {
                        ...
                        serviceScope.launch {
                            val walletMetadataMap = (application as ZeeyGoApplication).globalPrefs.getAvailableWalletsMeta.first()
