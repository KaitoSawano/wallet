...
object Logging {
    ...
    private const val CURRENT_LOG_FILE = "zeeygo.log"
    private const val ARCHIVED_LOG_FILE = "zeeygo.archive-%i.log"

    fun exportLogFile(context: Context): File {
        val export = File(File(context.cacheDir, LOGS_EXPORT_DIR).also { if (!it.exists()) it.mkdir() },
            "zeeygo_logs-${UUID.randomUUID().toString().substringBefore("-")}.log")
        ...
        File(File(context.filesDir, LOGS_DIR), "zeeygo.archive-1.log").takeIf {
    ...
    fun setupLogger(context: Context) {
        // cleanup export file
        val export = File(File(context.cacheDir, LOGS_DIR), "zeeygo_export.log")
        ...
        lc.getLogger("fr.acinq.zeeygo").level = if (BuildConfig.DEBUG) Level.DEBUG else Level.INFO
