package fr.acinq.zeeygo.android.settings.troubleshoot
...
import fr.acinq.zeeygo.android.utils.Logging.LOGS_EXPORT_DIR
...
import fr.acinq.zeeygo.utils.diagnostics.DiagnosticsHelper
...
class DiagnosticsViewModel(val application: ZeeyGoApplication, val business: ZeeyGoBusiness, val walletId: WalletId) : ViewModel() {
    ...
    fun shareDiagnostics(context: Context) {
        ...
        viewModelScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val file = File(File(context.cacheDir, LOGS_EXPORT_DIR).also { if (!it.exists()) it.mkdir() },
                "zeeygo_diagnostics-${UUID.randomUUID().toString().substringBefore("-")}.txt")
    ...
    class Factory(
        private val application: ZeeyGoApplication,
        private val business: ZeeyGoBusiness,
