...
import fr.acinq.zeeygo.android.security.EncryptedData
import fr.acinq.zeeygo.android.utils.converters.DateFormatter.toAbsoluteDateTimeString
import fr.acinq.zeeygo.csv.WalletPaymentCsvWriter
import fr.acinq.zeeygo.managers.DatabaseManager
...
class PaymentsExportViewModel(
    ...
    fun generateCSV(context: Context) {
        viewModelScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val file = File.createTempFile("zeeygo-", ".csv", exportDir)
    ...
    fun vacuumDatabase(context: Context, uri: Uri) {
        viewModelScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val exportDir = File(context.cacheDir, "payments")
            if (!exportDir.exists()) exportDir.mkdir()
            val file = File.createTempFile("zeeygo-payments-db-", ".sqlite", exportDir)
